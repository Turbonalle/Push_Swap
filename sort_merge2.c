...
void org(t_list *data, int size1)
{
	printf("Entered org(%d)!\n", size1);
	int rotations;

	rotations = 0;
	while (data->i_max_b >= 0)
	{
		while (rotations < size1 && data->stack_a[data->i_max_a] < data->stack_b[data->i_max_b])
		{
			rotate(data, 'a');
			rotations++;
		}
...
/*
Have to figure out when to use size1 and size2.
...
void sm(t_list *data, int size, int lap)
{
	...
	if (!o2(data, size2))
	{
		r(data, size2);	// s2 works with 9 numbers, s1 works with 10 numbers.
		sm(data, size2, lap + 1);
	}
